...
func (e *Expirer[Key]) startIfNeededLocked() {
	...
	go func() {
		for {
			...
			// Note: It is possible that a key being removed here might be updated
			// with a new expiration in the period between the scan and the
			// removal. The window during which this can happen is small, and the
			// stakes are low (keys may be evicted at any time anyway), so we
			// won't bother with additional sync.
