...
var rng = sync.OnceValue(func() *mrand.ChaCha8 {
	var seed [32]byte
	crand.Read(seed[:])
	return mrand.NewChaCha8(seed)
})

// Random fills data with non-cryptographic randomly-generated bytes.
func Random(data []byte) { rng().Read(data) }

// CaptureStdout temporarily replaces [os.Stdout] and runs f. Any output
// written to stdout by f is captured into w. It restores [os.Stdout] before
// returning, and returns the error, if any, reported by f.
func CaptureStdout(w io.Writer, f func() error) error {
	pr, pw, err := os.Pipe()
	if err != nil {
		return err
	}
	...
	go func() { defer close(done); io.Copy(w, pr) }()
